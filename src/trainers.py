    ...
    def iteration(self, epoch, dataloader, full_sort=True, train=True):
        ...
        if train:
            ...
            for i, (rec_batch, cl_batches) in rec_cf_data_iter:
                ...
                # ---------- generator task ---------------#
                # sequence_output = self.generator.transformer_encoder(input_ids)
                # gen_loss = self.generator_cross_entropy(sequence_output, target_pos, target_neg)
                # sampled_neg_ids, pos_idx, neg_idx, mask_idx, istarget = self.sample_from_generator(sequence_output,
                #                                                                                    target_pos)
                ...
                sequence_output_C = self.generator_C.transformer_encoder(sampled_neg_ids_A)
                ...
                # ---------- discriminator task ---------------#
                # disc_logits = self.discriminator.transformer_encoder(sampled_neg_ids)
                ...
                disc_logits_C = self.discriminator_C.transformer_encoder(sampled_neg_ids_C)
                ...
                joint_loss = self.args.gen_loss_weight * gen_loss + self.args.dis_loss_weight * dis_loss + cl_loss_gen * 0.55 + 0.5 * cyc_loss
            ...
            if full_sort:
                ...
                for i, batch in rec_data_iter:
                    ...
                    recommend_output = self.discriminator_A.transformer_encoder(input_ids)
